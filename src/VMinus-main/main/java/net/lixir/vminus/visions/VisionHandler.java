...
public class VisionHandler {
    ...
    private static final ConcurrentHashMap<String, Identifier> RESOURCE_LOCATION_CACHE = new ConcurrentHashMap<>();
    ...
    private static Identifier getOrCreateIdentifier(String tagNamespace) {
        return RESOURCE_LOCATION_CACHE.computeIfAbsent(tagNamespace, Identifier::new);
    }

    private static boolean isItemTagged(ItemStack itemstack, String tag) {
        String tagNamespace = tag.substring(1);
        TagKey<Item> itemTag = ITEM_TAG_CACHE.computeIfAbsent(tagNamespace, ns -> ItemTags.create(getOrCreateIdentifier(ns)));
    ...
    private static boolean isBlockTagged(Block block, String tag) {
        ...
        TagKey<Block> blockTag = BLOCK_TAG_CACHE.computeIfAbsent(tagNamespace, ns -> BlockTags.create(getOrCreateIdentifier(ns)));
    ...
    private static boolean isEntityTagged(EntityType<?> entity, String tag) {
        ...
        String tagNamespace = tag.substring(1);
        TagKey<EntityType<?>> entityTag = ENTITY_TAG_CACHE.computeIfAbsent(tagNamespace, ns -> TagKey.create(Registries.ENTITY_TYPE, getOrCreateIdentifier(ns)));
