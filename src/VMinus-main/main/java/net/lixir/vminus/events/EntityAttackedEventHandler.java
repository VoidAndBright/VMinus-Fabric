...
public class EntityAttackedEventHandler {
    ...
    public static void onEntityAttacked(LivingAttackEvent event) {
        if (event != null && event.getEntity() != null) {
            ...
            List<Identifier> particles = new ArrayList<Identifier>();
            ...
            if (entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new Identifier("vminus:invincible")))) {
                ...
                if (!mainhand.isEmpty() && mainhand.isEnchanted()) {
                            ...
                            if (visionData != null) {
                                if (visionData.has("particle")) {
                                    ...
                                    if (!particleString.isEmpty() && particleString != null) {
                                        Identifier particleLocation = new Identifier(particleString);
                                ...
                                if (visionData.has("sound")) {
                                    ...
                                    if (!world.isClientSide())
                                        world.playSound(null, BlockPos.containing(sourceentity.getX(),
                                                        sourceentity.getY() + 1,
                                                        sourceentity.getZ()),
                                                Objects.requireNonNull(ForgeRegistries.SOUND_EVENTS.getValue(new Identifier(soundString))),
                                                SoundSource.PLAYERS,
                                                (float) 0.8,
                                                (float) 1);
                                }
                    ...
                    if (particles.size() > 0) {
                        for (int index0 = 0; index0 < Mth.nextInt(RandomSource.create(), 5, 7); index0++) {
                            Identifier chosenParticle = particles.get(Mth.nextInt(RandomSource.create(), 0, (particles.size() - 1)));
