                ...
                if (tag == null || !tag.contains("broken")) {
                    ...
                    for (JsonElement element : attributesArray) {
                        ...
                        if (attributeData.has("id") && attributeData.has("value")) {
                            ...
                            Attribute attribute = ForgeRegistries.ATTRIBUTES.getValue(new Identifier(attributeId));
                            if (attribute != null) {
                                UUID attributeUUID;
                                ...
                                if (attributeData.has("slot")) {
                                    ...
                                    if (itemstack.is(ItemTags.create(new Identifier("forge", "armors/helmets")))) {
                                        slot = EquipmentSlot.HEAD;
                                    } else if (itemstack.is(ItemTags.create(new Identifier("forge", "armors/chestplates")))) {
                                        slot = EquipmentSlot.CHEST;
                                    } else if (itemstack.is(ItemTags.create(new Identifier("forge", "armors/leggings")))) {
                                        slot = EquipmentSlot.LEGS;
                                    } else if (itemstack.is(ItemTags.create(new Identifier("forge", "armors/boots")))) {
        ...
        if (ModList.get().isLoaded("detour")) {
            if (itemstack.isEnchantable()) {
                ...
                if (!tag.contains("enchantment_limit")) {
                    int limit = 2;
                    if (itemstack.is(ItemTags.create(new Identifier("detour:one_enchantment_limit")))) {
                        limit = 1;
                    } else if (itemstack.is(ItemTags.create(new Identifier("detour:three_enchantment_limit")))) {
                        limit = 3;
                    } else if (itemstack.is(ItemTags.create(new Identifier("detour:four_enchantment_limit")))) {
                        limit = 4;
                    }
